	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		if (!empty($_POST["seller"]) && !empty($_POST["category"]) && !empty($_POST["description"]) && !empty($_POST["quality"]) && !empty($_POST["price"])) {
			...
			if($_POST["category"] == "books") {
				if(!empty($_POST["author"]) && !empty($_POST["edition"]) && !empty($_POST["branch"]) && !empty($_POST["sem"])){
					$category =  "books";//mysqli_real_escape_string($conn, sanitize_input($_POST["category"]));
			...
			if($_POST["category"] == "bikes"){
				if(!empty($_POST["brand"]) && !empty($_POST["gear"]) && !empty($_POST["colour"])){
					$category =  "bikes";//mysqli_real_escape_string($conn, sanitize_input($_POST["category"]));
			...
			if($_POST["category"] == "misc"){
				if(!empty($_POST["author"]) && !empty($_POST["branch"]) && !empty($_POST["sem"])){
					$category =  "misc";//mysqli_real_escape_string($conn, sanitize_input($_POST["category"]));
					$author =  mysqli_real_escape_string($conn, sanitize_input($_POST["author"]));
					$branch =  mysqli_real_escape_string($conn, sanitize_input($_POST["branch"]));
					$sem =  mysqli_real_escape_string($conn, sanitize_input($_POST["sem"]));
				}
			...
			if($category == "books"){ //Update the books table with this value
				$result = mysqli_query($conn,"INSERT INTO books (author,edition,seller,branch,sem,description,quality,price) VALUES ('".$author."','".$edition."','".$seller."','".$branch."','".$sem."','".$description."','".$quality."','".$price."')");

				$sale_id = mysqli_query($conn,"SELECT id FROM salerequest WHERE seller = '".$seller."' AND author = '".$author."' AND edition = '".$edition."' AND branch = '".$branch."' AND sem = '".$sem." AND description = '".$description."' AND quality = '".$quality."' AND price = '".$price"'");//Fetch the id of the above entry
